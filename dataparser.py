...
try:
    ...
    with open(file_path, 'r', encoding='utf8') as file:
        ...
        #read in the file and replace all quotations
...
for s in sentences:
    ...
    for c in characters:
        arffRow.append(float(chars[c]) / len(s))
    
    #add word length average too the arff row
    arffRow.append(len(s.replace(' ', '')) / float(numWords))
    
    #add the number of words to the arff row
    arffRow.append(numWords)

    #add the current language being trained to the arff row
    #TODO change this to the name of the language you are training!
